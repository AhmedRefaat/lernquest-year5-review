# Catch the Falling Stars Architecture

## Purpose
`falling_stars` is a self-contained LernQuest mini-game. It consumes a parent-provided play-time budget, pauses automatically when that budget reaches zero, persists all durable progress, and resumes from the saved state when the learner later earns more time.
...
`lernquest-game:falling_stars:<playerId>`
...
## Durable state
Score, caught-item count, best score, player position, total play time, rounds, and last-played timestamp.
