# Balloon Pop Challenge Architecture

## Purpose
`balloon_pop` is a self-contained LernQuest mini-game. It consumes a parent-provided play-time budget, pauses automatically when that budget reaches zero, persists all durable progress, and resumes from the saved state when the learner later earns more time.
...
`lernquest-game:balloon_pop:<playerId>`
...
## Durable state
Score, best combo, total balloons popped, total play time, rounds, and last-played timestamp.
