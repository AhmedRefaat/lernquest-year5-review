# Feed and Grow Your Pet Architecture

## Purpose
`happy_pet` is a self-contained LernQuest mini-game. It consumes a parent-provided play-time budget, pauses automatically when that budget reaches zero, persists all durable progress, and resumes from the saved state when the learner later earns more time.
...
`lernquest-game:happy_pet:<playerId>`
...
## Durable state
Pet identity, XP, level, fullness, unlocked accessories, equipped accessory, total play time, rounds, and last-played timestamp.
