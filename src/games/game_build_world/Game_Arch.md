# Build Your Own World Architecture

## Purpose
`build_world` is a self-contained LernQuest mini-game. It consumes a parent-provided play-time budget, pauses automatically when that budget reaches zero, persists all durable progress, and resumes from the saved state when the learner later earns more time.
...
`lernquest-game:build_world:<playerId>`
...
## Durable state
Coin balance, 24-tile world layout, unlocked items, placed-item count, total play time, rounds, and last-played timestamp.
